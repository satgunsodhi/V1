...
// Class to represent transportation options
class Transportation {
public:
    string mode;
    double cost_per_person;

    Transportation(const string& mode, double cost_per_person) : mode(mode), cost_per_person(cost_per_person) {}
};

// Class to represent accommodation options
class Accommodation {
public:
    string name;
    double cost_per_room;

    Accommodation(const string& name, double cost_per_room) : name(name), cost_per_room(cost_per_room) {}
};
...
public:
    Trip(const string& destination, const Transportation& transport, const Accommodation& accommodation, int num_people, int num_rooms, const string& itinerary)
        : destination(destination), transport(transport), accommodation(accommodation), num_people(num_people), num_rooms(num_rooms), itinerary(itinerary) {
        total_cost = calculateTripCost();
    }

    // Function to calculate the cost of a trip
    double calculateTripCost() const {
        return transport.cost_per_person * num_people + accommodation.cost_per_room * num_rooms;
    }
...
int main() {
    ...
    Accommodation budgetHotel("Budget Hotel", 50.0);
    Accommodation standardHotel("Standard Hotel", 100.0);
    Accommodation luxuryHotel("Luxury Hotel", 200.0);

    // Create an array to store pre-made packages
    const int numPackages = 9;
    Trip packages[numPackages] = {
        Trip("Shimla", car, budgetHotel, 2, 1, "Example Itinerary for Shimla:\nDay 1: Arrival and check-in at the hotel.\nDay 2: Sightseeing around the city.\nDay 3: Departure."),
        Trip("Shimla", train, standardHotel, 4, 2, "Example Itinerary for Shimla:\nDay 1: Arrival and check-in at the hotel.\nDay 2-3: Explore local attractions.\nDay 4: Departure."),
        Trip("Shimla", plane, luxuryHotel, 6, 3, "Example Itinerary for Shimla:\nDay 1: Arrival and transfer to the hotel.\nDay 2-4: Enjoy leisure activities and city tours.\nDay 5: Departure."),
        Trip("Manali", car, standardHotel, 3, 2, "Example Itinerary for Manali:\nDay 1: Arrival and check-in at the hotel.\nDay 2: Explore nearby attractions.\nDay 3: Departure."),
        Trip("Manali", train, luxuryHotel, 2, 1, "Example Itinerary for Manali:\nDay 1: Arrival and transfer to the luxury hotel.\nDay 2: Enjoy spa and relaxation.\nDay 3: Departure."),
        Trip("Manali", plane, budgetHotel, 5, 3, "Example Itinerary for Manali:\nDay 1: Arrival and check-in at the budget hotel.\nDay 2-4: Visit tourist attractions.\nDay 5: Departure."),
        Trip("Goa", car, standardHotel, 3, 2, "Example Itinerary for Goa:\nDay 1: Arrival and check-in at the hotel.\nDay 2: Beach activities.\nDay 3: Departure."),
        Trip("Goa", train, luxuryHotel, 2, 1, "Example Itinerary for Goa:\nDay 1: Arrival and transfer to the luxury hotel.\nDay 2: Explore the nightlife.\nDay 3: Departure."),
        Trip("Goa", plane, budgetHotel, 4, 2, "Example Itinerary for Goa:\nDay 1: Arrival and check-in at the hotel.\nDay 2-3: Water sports.\nDay 4: Departure.")
    };
